# Codey – Product Requirements Document

| Položka | Hodnota |
| --- | --- |
| Dokument | Product Requirements Document |
| Verze | 0.1 |
| Stav | První úplná verze k odborné a autorské revizi |
| Datum | 10. srpna 2026 |
| Produkt | Codey |
...
Tento dokument definuje produktové požadavky na Codey, desktopový AI coding agent pro práci nad lokálními softwarovými projekty. Slouží jako společný podklad pro návrh architektury, etapizaci vývoje, měsíční konzultace, testování, školní dokumentaci a obě obhajoby. Nejde o implementační plán ani o závazný návrh vizuální identity.
...
2. aktuální zadání autora projektu, včetně potvrzeného názvu Codey;
...
- **P-01:** Codey je pracovní i produktový název pro PRD v0.1.
...
Codey je desktopový AI coding agent pro zkušenější vývojáře, který pracuje nad jejich lokálními projekty. Uživatel zadává úkol v chatu, agent si cíleně získává kontext, volá bezpečně zpřístupněné nástroje, upravuje kód, spouští relevantní kontroly a dokládá výsledek pomocí událostí, diffu a testovacích výstupů. Uživatel může práci sledovat, řídit oprávnění, změny vracet a zvolit si poskytovatele i model pomocí vlastního API klíče.

Codey není pouze chatové rozhraní nad LLM API. Hlavní produktovou hodnotou je vlastní agentní infrastruktura: orchestrátor, modelové adaptéry, nástroje, bezpečný execution harness, pravidla oprávnění, event log, správa kontextu a ověřovací smyčka. Kvalita výsledku nemá stát jen na slibu modelu, že je úkol hotový, ale na pozorovatelném pracovním procesu a konkrétním důkazu.

### 1.1 Pracovní elevator pitch

> Codey je transparentní desktopový AI coding agent, který nad lokálním projektem nejen navrhuje změny, ale bezpečně je provede, ověří a ukáže důkazy, takže vývojář získá vysokou míru automatizace bez ztráty kontroly.
...
Codey může propojit několik známých mechanismů do jednoho srozumitelného pracovního prostředí:
...
Originalita nespočívá v tvrzení, že žádný dílčí mechanismus jinde neexistuje. Spočívá ve vlastním návrhu harnessu, integraci uvedených prvků, měřitelném ověření jejich přínosu a ve způsobu, jakým Codey dává uživateli vysokou autonomii i kontrolu současně.
...
| Oblast | Běžné riziko | Zamýšlené odlišení Codey |
| --- | --- | --- |
...
První obhajoba musí ukázat malou, stabilní a projektorem dobře čitelnou svislou cestu. Cílem není dokončit většinu obrazovek, ale přibližně 65 % plánované funkčnosti agentního harnessu a naprostou většinu tool callů potřebných pro základní reálný úkol.
...
**Může být z O1 přesunuto bez porušení hlavního cíle:** plná autentizace, cloudová synchronizace, druhý reálný modelový adapter, Playwright vision-check, pokročilá LLM Wiki, Auto režim s klasifikátorem, online marketplace a finální vizuální zpracování.
...
- cloudová synchronizace chatů a nastavení v rozsahu, který neohrozí lokální jádro; pokud úplná synchronizace nesplní quality gate, školní verze musí alespoň bezpečně demonstrovat omezený synchronizační tok a jasně jej označit;
...
- handoff snapshot rozpracovaného stromu mezi zařízeními;
...
| --- | --- | --- |
| FR-DESK-01 | M / O1 | Codey musí být spustitelná desktopová aplikace na podporované instalaci Windows 11. |
...
| FR-PROJ-02 | M / ŠF | Uživatel může vytvořit záznam nového projektu a zvolit jeho prázdnou nebo existující lokální složku; Codey nemusí scaffoldovat aplikační framework. |
...
| FR-PROJ-04 | M / O1 | Codey musí určit, zda otevřená složka obsahuje Git repozitář, a zobrazit aktivní režim práce. |
...
| FR-PROJ-08 | S / ŠF | Codey má rozpoznat TypeScript/Node.js, React a Next.js profil podle konfiguračních souborů a závislostí, aniž by je měnil. |
...
| FR-CHAT-06 | M / ŠF | Uživatel může pokračovat v dřívějším chatu a Codey musí sestavit kontext podle pravidel, ne automaticky odeslat celou historii a repozitář. |
...
| FR-TOOL-05 | M / O1 | Příkaz musí běžet v explicitně určeném pracovním adresáři s timeoutem a zachyceným stdout, stderr a exit code. |
...
| FR-TOOL-10 | M / O1 | Běžící proces musí jít zrušit a po timeoutu musí Codey ukončit podporovaný strom potomků nebo zřetelně hlásit proces, který ukončit nedokázal. |
...
| FR-PERM-05 | M / ŠF | Auto režim musí každou nevyřešenou akci klasifikovat; při nejistotě musí přejít k uživatelskému schválení. |
...
| ID | Priorita / etapa | Požadavek |
| --- | --- | --- |
| FR-CHG-01 | M / O1 | Před první agentní změnou musí Codey vytvořit obnovovací bod pro dotčený obsah nebo ověřit použitelný Git základ. |
...
| FR-CHG-06 | M / O1 | Při změně souboru od vytvoření obnovovacího bodu musí Codey detekovat konflikt a vyžádat řešení místo slepého přepsání. |
...
| ID | Priorita / etapa | Požadavek |
| --- | --- | --- |
| FR-VCS-01 | M / O1 | Při otevření projektu musí Codey detekovat, zda je kořen uvnitř Git worktree, a uložit výsledek k relaci. |
| FR-VCS-02 | M / O1 | V Git režimu musí Codey před prací zachytit výchozí commit, branch a stav pracovního stromu. |
...
| FR-VCS-04 | M / O1 | V non-Git režimu musí Codey vytvořit vlastní obsahový snapshot dotčených souborů a umožnit obnovu. |
...
| FR-VCS-08 | C / PŠ | Handoff snapshot může spojit identitu chatu, remote, výchozí commit, testovací stav a uživatelem zkontrolovaný patch bez ignorovaných tajemství. |

### 8.11 Terminál, náhled a ověřovací smyčka

| ID | Priorita / etapa | Požadavek |
| --- | --- | --- |
| FR-TERM-01 | M / O1 | Codey musí zobrazit příkaz spuštěný agentem, jeho pracovní adresář, průběžný výstup, exit code a délku běhu. |
...
| FR-TERM-04 | M / O1 | Pro rozpoznaný TypeScript projekt musí Codey nabídnout nebo odvodit relevantní kontrolu typů a testovací příkaz, ale nesmí bez potvrzení instalovat chybějící závislosti. |
...
| FR-TERM-08 | S / ŠF | Codey může pomocí Playwrightu otevřít nakonfigurovanou lokální URL, pořídit screenshot a připojit metadata viewportu a času. |
...
| ID | Priorita / etapa | Požadavek |
| --- | --- | --- |
| FR-TS-01 | S / O1 | O1 může obsahovat technický prototyp strukturálního dotazu; jeho absence nesmí blokovat základní agentní cyklus. |
| FR-TS-02 | M / ŠF | TypeScript profil musí najít deklaraci symbolu podle názvu a umístění a vrátit cestu, rozsah a stručný druh symbolu. |
| FR-TS-03 | M / ŠF | Profil musí umět vrátit exporty souboru, signaturu funkce nebo metody a nalezené reference v omezeném strukturovaném JSON výstupu. |
| FR-TS-04 | M / ŠF | Strukturální dotaz musí respektovat `tsconfig.json`, pokud je dostupný, a jasně označit fallback při neúplném nebo neplatném projektu. |
| FR-TS-05 | M / ŠF | Model nesmí dostat celý serializovaný AST; nástroj vrací pouze data potřebná pro konkrétní dotaz s limitem výsledků. |
| FR-TS-06 | M / ŠF | Při selhání `ts-morph` nebo Compiler API musí zůstat dostupné běžné čtení a textové hledání. |
| FR-TS-07 | M / ŠF | Výsledky strukturálních dotazů musí být invalidovány po změně relevantních souborů nebo konfigurace projektu. |
| FR-TS-08 | S / ŠF | Přínos strukturálního nástroje musí být změřen na sadě nejméně 20 dotazů proti ručně ověřenému očekávání. |
| FR-TS-09 | C / PŠ | LSP nebo TypeScript language server může doplnit Compiler API, pokud prokáže přesnější reference či typy bez nepřijatelné provozní složitosti. |
...
| FR-CTX-03 | S / O1 | O1 může uchovávat jednoduché ručně nebo deterministicky získané projektové shrnutí a seznam důležitých souborů. |
| FR-CTX-04 | M / ŠF | Repo mapa musí uchovávat nejméně cestu, druh souboru, hlavní exporty nebo symboly, stručnou roli a vazbu na aktuální obsahovou revizi. |
| FR-CTX-05 | M / ŠF | Změna souboru musí označit jeho odvozené shrnutí jako zastaralé nejpozději před dalším sestavením kontextu. |
| FR-CTX-06 | M / ŠF | Uživatel musí vidět alespoň stručný přehled, jaké zdroje kontextu byly pro požadavek vybrány. |
| FR-CTX-07 | M / ŠF | Projektová pravidla s vyšší autoritou musí být oddělena od modelově generovaných shrnutí, aby shrnutí nemohlo pravidla přepsat. |
| FR-CTX-08 | M / ŠF | Prototyp LLM Wiki musí být porovnán s baseline bez wiki na stejné sadě úloh podle tokenů, počtu kroků a věcné úspěšnosti. |
| FR-CTX-09 | S / ŠF | Staré části konverzace mohou být zkomprimovány do pracovní paměti, ale původní lokální historie musí zůstat dohledatelná pro uživatele. |
| FR-CTX-10 | C / PŠ | Provider-native context caching může být využito adaptérem, nesmí však být podmínkou správného chování orchestrace. |
...
| FR-CAN-06 | M / ŠF | Pokud zvolený model nepodporuje obrazový vstup, Codey musí odeslání zastavit nebo nabídnout jasně označenou textovou alternativu; nesmí předstírat, že model obrázek viděl. |
...
Pro Codey platí následující pracovní terminologie:
...
| ID | Priorita / etapa | Požadavek |
| --- | --- | --- |
| FR-EXT-01 | M / O1 | Codey musí mít registry vestavěných skillů a pluginových manifestů se jménem, verzí, popisem, schopnostmi a požadovanými oprávněními. |
...
| FR-EXT-07 | M / ŠF | Před prvním připojením MCP serveru musí Codey zobrazit příkaz nebo URL, proměnné bez hodnot tajemství a požadované schopnosti. |
...
| FR-ACC-05 | S / ŠF | Cloudová synchronizace má zahrnout chaty, zprávy, uživatelská nastavení a projektová metadata, nikoli automaticky obsah lokálních zdrojových souborů. |
...
| FR-ACC-08 | S / ŠF | Synchronizace musí zobrazit alespoň stavy lokální, čeká na odeslání, synchronizováno, offline a konflikt. |
| FR-ACC-09 | S / ŠF | Konflikt podporované entity se nesmí tichým last-write-wins chováním ztratit; musí být deterministicky sloučen nebo předložen uživateli podle typu dat. |
| FR-ACC-10 | M / ŠF | Uživatel musí požádat o export nebo smazání podporovaných cloudových dat a vidět dopad na lokální kopii. |
| FR-ACC-11 | C / PŠ | Handoff zdrojových změn je samostatná vědomá akce s diffem, kontrolou klonu, vyloučením ignorovaných/citlivých souborů a bezpečným řešením konfliktu. |
...
**Hlavní tok:** Uživatel spustí Codey, projde krátkým vysvětlením lokálního přístupu a BYOK, zvolí Řízený režim a pracovní plochu Stavitel, provider může nastavit nebo krok odložit. Codey uloží volbu a otevře domovskou obrazovku.  
...
**Hlavní tok:** Codey označí non-Git režim, před editací vytvoří vlastní snapshot, provede a ověří změnu a zobrazí vlastní diff. Uživatel vyvolá návrat celé relace.  
...
**Hlavní tok:** Codey zobrazí přesný příkaz, pracovní adresář a důvod. Uživatel jej zamítne. Harness příkaz nespustí, zaznamená zamítnutí a model dostane strukturovanou informaci, aby hledal bezpečnou alternativu.  
...
**Hlavní tok:** Uživatel požádá o vrácení agentního patchu. Codey zjistí nesoulad s očekávanou revizí, návrat neprovede automaticky a zobrazí konflikt.  
...
**Hlavní tok:** Uživatel otevře obrázek na canvasu, zakroužkuje prvek a textem popíše změnu. Codey zobrazí náhled payloadu a předá původní obrázek, anotaci a text multimodálnímu adapteru.  
...
**Hlavní tok:** Uživatel vybere složku. Codey manifest pouze načte a zobrazí název, verzi, příkazy, endpointy a oprávnění. Uživatel import zruší nebo potvrdí registraci.  
...
**Hlavní tok:** Uživatel se přihlásí, Codey načte podporované chaty, nastavení a projektová metadata. Lokální cesta projektu není automaticky považována za platnou; uživatel může projekt znovu přiřadit.  
...
**Hlavní tok:** Codey zobrazí přesnou kategorii chyby a zachová rozpracovaný stav. Autor přepne na předem připravený deterministický scénář se stejnými obrazovkami, tool cally, diffem a testem.  
...
| --- | --- | --- |
| AC-O1-01 | Na čistém podporovaném testovacím účtu Windows 11 lze Codey spustit a otevřít referenční projekt nejvýše do 2 minut od zahájení demonstrace. | Časovaný záznam z generální zkoušky |
...
| Desktop backend | Electron main proces a oddělené Node.js worker/child procesy podle rizika | Izolace privilegovaných operací a dlouhých tool callů |
| Orchestrace | TypeScript/Node.js | Jednotný stack a vhodné asynchronní API |
| Lokální perzistence | Relační embedded databáze po krátkém spike | Vztahy mezi chaty, událostmi, změnami a migrace; konkrétní databáze je otevřené rozhodnutí |
...
Electron je pracovní volba, nikoli neodvolatelné dogma. Krátké technické ověření musí před implementací širokého UI potvrdit bezpečné IPC, spouštění a rušení procesů, ukládání tajemství a velikost/distribuci aplikace. Tauri se zvažuje pouze tehdy, pokud spike odhalí zásadní překážku, ne kvůli obecné optimalizaci velikosti.
...
Auto režim není „bezpečnost vypnuta“. Je to jiný způsob rozhodování uvnitř stejných pevných hranic. Klasifikátor smí automaticky povolit pouze akci, pro kterou jsou splněna pevná pravidla, deklarovaný rozsah a dostatečná jistota. Neznámá, víceznačná nebo nově kombinovaná akce přechází k uživateli. Přesná implementace klasifikátoru zůstává otevřeným technickým rozhodnutím a musí projít eval sadou bezpečných a rizikových akcí.

### 13.5 Soukromí vůči LLM poskytovateli

Před prvním použitím provideru Codey vysvětlí, že text, kód a obrázky mohou být odeslány externí službě a že podmínky bezplatného a placeného režimu se mohou lišit. PRD nestanovuje aktuální právní nebo cenové tvrzení; text v produktu musí být před vydáním ověřen z primární dokumentace poskytovatele. Uživatel musí vědět, který provider a model je pro relaci aktivní.
...
1. Otevřít připravený lokální TypeScript/React projekt s malou aplikací, například hrou šibenice.
2. Ukázat, že Codey rozpoznal projekt a Git/non-Git stav.
3. Zadat malý a vizuálně pochopitelný úkol, například opravu chyby nebo přidání omezené funkce.
...
Tato část obsahuje pouze rozhodnutí, pro která v podkladech skutečně chybí dostatečný závazný podklad. Nejde o blokaci sepsání PRD; každé rozhodnutí má okamžik, do kterého má být uzavřeno.

### OD-01: Finální potvrzení Electronu

**Co chybí:** výsledek krátkého spike na bezpečné IPC, child procesy, credential storage a distribuci Windows buildu.  
**Výchozí směr:** Electron.  
**Kritérium rozhodnutí:** Electron zůstává, pokud spike zvládne povinné procesní a bezpečnostní fixture bez zásadního workaroundu.  
**Termín:** před širokým scaffoldem desktopové architektury.

### OD-02: Lokální databáze a migrační mechanismus

**Co chybí:** porovnání knihoven z hlediska Electron balení, transakcí, migrací a Windows instalace.  
**Požadované vlastnosti:** embedded relační databáze, transakce, verzované migrace, záloha a dobrá TypeScript integrace.  
**Termín:** před implementací trvalého event logu.

### OD-03: Implementace klasifikátoru Auto režimu

**Možnosti:** čistá deterministická pravidla; pravidla s lokální heuristikou; pravidla s pomocným modelem.  
**Rozhodovací kritéria:** false-negative míra u rizikových akcí, latence, vysvětlitelnost, offline chování a náklady.  
**Výchozí bezpečný fallback:** při nejistotě `ask`; Auto režim může být zpočátku pravidlový.  
**Termín:** po dokončení Řízeného režimu a eval sady.

### OD-04: Algoritmus LLM Wiki/repo mapy

**Co chybí:** přesná metoda tvorby shrnutí, výběru kontextu a invalidace.  
**Minimální varianta:** deterministická TypeScript repo mapa s hlavními symboly a ručně ověřenými pravidly projektu.  
**Rozhodovací kritéria:** věcná úspěšnost, tokeny, počet kroků, latence a složitost údržby.  
**Termín:** po stabilní základní smyčce a TypeScript profilu.

### OD-05: Licence projektu

**Co chybí:** rozhodnutí, zda má být školní repozitář veřejný a jaká práva chce autor poskytnout.  
**Rozhodovací vstupy:** použité závislosti, případná publikační strategie a školní pravidla.  
**Termín:** před prvním veřejným vydáním nebo sdílením zdrojového kódu.

### OD-06: Závazný rozsah synchronizace pro ŠF

**Co chybí:** praktická náročnost konfliktů, příloh a offline fronty po Supabase spike.  
**Pevné minimum:** účet je součást ŠF; lokální jádro funguje bez cloudu; API klíče ani zdrojový kód se nesynchronizují.  
**Preferovaný rozsah:** chaty, zprávy, nastavení a projektová metadata.  
**Termín:** po autentizačním a dvouklientovém synchronizačním prototypu.

### OD-07: Hranice procesní izolace ve ŠF

**Co chybí:** zda všechny nástroje běží v child workeru, zda se vybrané nástroje oddělí podle rizika a zda je na Windows proveditelný silnější sandbox bez Docker závislosti.  
**Pevné minimum:** renderer je oddělený, shell má cwd/timeout/audit, nástroje nemohou obejít permission engine.  
**Termín:** po process runner spike, před připojením cizího MCP serveru.
...
| Pojem | Význam v Codey |
| --- | --- |
| Agentní relace / AgentRun | Jeden řízený běh od uživatelského požadavku po konečný stav |
| Adapter | Překlad mezi interním formátem Codey a API konkrétního LLM poskytovatele |
...
| Non-Git snapshot | Vlastní lokální obnovovací bod Codey pro projekt bez použitelného Gitu |
