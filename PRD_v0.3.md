...
| Verze | 0.3 |
| Stav | Technicky zpřesněná rozhodovací verze |
| Datum | 12. srpna 2026 |
...
### 0.2.1 Změny proti v0.2

PRD v0.3 zachovává produktové jádro v0.2 a technicky je zpřesňuje. Nově zejména:

- zavádí produktový baseline pro běžné UX chování a postup jeho doplňování během vývoje;
- přidává `@` reference na soubory a složky do O1 a rozlišuje je od nahraných příloh;
- rozšiřuje O1 o kompaktní Git workspace včetně commit, fetch, pull a push;
- zapracovává schválené minimum Workspace Intelligence, koordinace relací a subagentů;
- popisuje stavové automaty, vlastnictví dat, interní kontrakty a zakázané závislosti;
- nahrazuje předčasně pevnou volbu prvního živého modelu měřitelným provider evalem;
- zařazuje epizodickou paměť pouze jako offline experiment E0 s rozhodovací bránou;
- přidává nové scénáře a akceptační kritéria pro Git, kontext, souběh a recovery.
...
- **P-01:** Codryn je pracovní název pro PRD v0.3; před veřejným vydáním proběhne samostatná kontrola názvu a licence.
...
- **P-13:** Epizodická paměť není ve v0.3 schválenou funkcí O1. Závazný je pouze experiment E0 a rozhodnutí podle jeho výsledků.
...
- kompaktní Git workspace se stavem repozitáře, staged/unstaged soubory, diffem, historií commitů, vytvořením commitu a explicitními operacemi fetch, pull a push;
- základní Workspace State, koordinace souběžných relací a subagenti se stejnou hashovou ochranou zápisů;
- offline experiment E0 epizodické paměti bez rozšíření závazné produktové funkce;
...
| FR-VCS-15 | M / O1 | Uživatel musí explicitně spustit `fetch`, `pull` nebo `push`; UI musí před spuštěním zobrazit remote, branch a známý ahead/behind stav. |
| FR-VCS-16 | M / O1 | Před pull, přepnutím branche nebo jinou operací měnící worktree musí Codryn znovu zkontrolovat necommitnuté a aktivní agentní změny a při riziku ztráty nebo konfliktu operaci zastavit s vysvětlením. |
| FR-VCS-17 | M / O1 | Pull nesmí automaticky zvolit neznámou merge/rebase strategii; použije jednoznačně zjištěnou projektovou či Git konfiguraci, nebo si vyžádá uživatelskou volbu. |
| FR-VCS-18 | M / O1 | Push musí bezpečně odmítnout non-fast-forward stav a běžný tok O1 nesmí nabízet automatický force push. |
| FR-VCS-19 | M / O1 | Git operace musí využít existující Git credential mechanismus bez čtení nebo ukládání hesla či tokenu do Codryn databáze a event logu. |
...
| FR-COORD-03 | M / O1 | Aktivní relace musí publikovat strukturovaný `WorkIntent` s režimem, krátkým `taskSummary`, očekávanými oblastmi a známými sdílenými zdroji; jde o měkkou deklaraci, nikoli zámek. |
| FR-COORD-04 | M / O1 | `taskSummary` musí vznikat automaticky z aktuálního zadání a plánu, mít nejvýše jednu až dvě věty a být uživatelem viditelné, upravitelné nebo vypnutelné bez sdílení celého chatu. |
| FR-COORD-05 | M / O1 | Relace je pro koordinaci aktivní nejméně ve stavech běhu, čekání na schválení, zápisu, `recently_writing` a `idle_with_changes`; `recently_writing` trvá pět minut od posledního úspěšného zápisu. |
| FR-COORD-06 | M / O1 | Nový nebo pokračující agent musí před plánem dostat malý snapshot relevantních relací se stavem, task summary, očekávanými a změněnými oblastmi a branch/worktree metadaty, nikdy jejich plný chat. |
| FR-COORD-07 | M / O1 | Koordinační politika musí rozlišit nízký, možný a vysoký překryv a dovolit agentovi pokračovat, změnit plán, počkat nebo navrhnout worktree; vytvoření worktree vyžaduje souhlas uživatele. |
...
| FR-COORD-11 | M / O1 | Rodičovský agent musí vytvořit subagenta pro omezený úkol, zobrazit jej ve stromu práce a předat mu stejná nebo užší oprávnění. |
| FR-COORD-12 | M / O1 | Subagent smí přímo zapisovat do společného workspace, ale každý jeho zápis podléhá stejnému očekávanému hashi/revizi, auditní provenienci a resource-key serializaci. |
| FR-COORD-13 | M / O1 | Provenience subagenta musí obsahovat jeho ID, ID rodičovské relace, tool call a change set; rodič odpovídá za kontrolu a začlenění výsledku. |
...
| ID | Priorita / etapa | Požadavek |
| --- | --- | --- |
| FR-EXP-01 | M / O1 | E0 musí použít anonymizované nebo syntetické dlouhé coding relace s ručně označenými relevantními návraty, hranicemi a kanonickou evidencí. |
| FR-EXP-02 | M / O1 | Experiment musí na stejných otázkách porovnat nejméně plnou historii, posuvné okno, lineární kompakci a jednoduché strukturované checkpointy se selektivním dohledáním evidence. |
| FR-EXP-03 | M / O1 | E0 musí měřit správnost odpovědi, retrieval recall/precision, snížení aktivních tokenů, režii latence, detekci zastaralého stavu, kvalitu hranic a možnost opravy chybného checkpointu z originálních událostí. |
| FR-EXP-04 | M / O1 | Paměťová evidence se musí v experimentu považovat za nedůvěryhodná a historická data; tvrzení o aktuálním souboru, Git stavu, schématu nebo testu se ověřuje proti současnému zdroji. |
| FR-EXP-05 | M / O1 | Experimentální report musí uvést i neúspěšné případy, použitý dataset, tokenové počítání, model, konfiguraci a reprodukovatelný postup bez marketingového výběru výsledků. |
| FR-EXP-06 | M / O1 | Přechod k produktové variantě A vyžaduje, aby checkpointy zachovaly nebo zlepšily správnost, prokazatelně snížily aktivní kontext, zachytily připravené stale reference a zůstaly auditovatelné; jinak se návrh odloží nebo zamítne. |
| FR-EXP-07 | C / ŠF | SQLite FTS5, automatické hranice, modelový reranking nebo embeddings lze zařadit teprve samostatným rozhodnutím po E0; nejsou implicitní součástí v0.3. |
...
| AC-O1-21 | Fetch, pull a push proběhnou proti lokálnímu testovacímu remote; pull s rizikovými lokálními změnami a non-fast-forward push jsou bezpečně zastaveny a force push není v běžném O1 toku dostupný. | Git remote fixture |
| AC-O1-22 | Dvě relace nemohou současně provést konfliktní stavovou Git operaci nad stejným resource key, zatímco editace různých souborů mohou pokračovat paralelně. | Koordinační integrační test |
| AC-O1-23 | Patch samostatné relace i subagenta založený na neaktuálním hashi je odmítnut před zápisem, zapíše auditní událost a zachová novější obsah beze změny. | Concurrency fixture a kontrola hashů |
...
| AC-SF-19 | Obnovená relace po změně HEAD nebo relevantního souboru zobrazí rozdílový snapshot a nepoužije staré ověření jako aktuální důkaz. | Recovery integrační test |
...
13. Před dalším krokem porovná použitou workspace revizi s živým stavem a zneplatní dotčený kontext nebo verification record.
...
Povolený hlavní tok je `idle → preparing_context → waiting_for_model → executing_tool | waiting_for_approval | verifying → waiting_for_model | completed | cancelled | failed`. Přechod do `completed` je možný jen podle definice dokončení. Zrušení je požadavek, dokud backend nepotvrdí ukončení aktivní operace. Restart rendereru stav nemění; restart backendu může obnovit pouze explicitně podporovaný konzistentní stav.
...
| WorkIntent | Krátký sdílený záměr relace, očekávané oblasti a resource keys | lokálně |
...
| SubagentRun | Identita podúkolu, rodič, oprávnění, stav a provenance výsledku | lokálně |
| ContextExperimentRun | Konfigurace, dataset, baseline, metriky a výsledek E0 | lokálně, report exportovatelný |
...
- lokální bare remote a dva klony pro fetch/pull/push, ahead/behind a non-fast-forward scénáře;
...
- dvě souběžné relace se shodným i rozdílným souborem;
- subagent s patchem založeným na zastaralém hashi;
- lockfile, Git a databázové resource-key kolize;
- zastaralý verification record po relevantní i nerelevantní změně;
- anonymizované nebo syntetické dlouhé relace pro E0.
...
E0 běží odděleně od produktového UI. Dataset, ruční anotace, baseline konfigurace, model, token counter a dotazy se verzují. Každý běh zachová surové výsledky a odvozené metriky. Report musí samostatně uvést případy nesprávného retrievalu, zastaralé odpovědi, over-boxing a situace, kdy jednoduchá kompakce překonala checkpointy. Rozhodnutí pokračovat není založeno pouze na tokenové úspoře, pokud se zhoršila správnost nebo auditovatelnost.
...
| Git workspace přeroste hlavní produkt | střední / vysoký | O1 omezený na každodenní tok, pokročilé rebase/cherry-pick/stash funkce mimo povinný rozsah | Git práce odsouvá dokončení harnessu nebo nemá stabilní remote fixture |
| Pull/push poškodí práci nebo historii | nízká až střední / vysoký | Čerstvý preflight, explicitní akce, zákaz automatického force push, lokální remote testy | Jakákoli fixture ztratí lokální změnu nebo přepíše vzdálenou historii |
...
| Experimentální paměť zhorší správnost | střední / střední | E0 před implementací, kanonické eventy, freshness validation, zveřejnění selhání | Tokeny klesnou, ale odpovědi nebo stale detekce jsou horší než baseline |

## 20. Otevřená rozhodnutí

Většina dřívějších rozhodnutí byla uzavřena a jejich důvody jsou vedeny v `REGISTR_ROZHODNUTI_v0.3.md`. Zde zůstávají pouze volby, které nelze kvalitně uzavřít bez implementačních dat.
...
**Termín:** před uzavřením F1 a použitím živého provideru jako výchozího demonstračního toku.

### OD-05: Pokračování epizodické paměti po E0

**Uzavřený směr:** E0 je povinný technický experiment, ale epizodická paměť není funkcí O1.  
...
**Termín:** E0 dokončit během F2 nejpozději před zmrazením rozsahu O1.

### OD-06: Výchozí strategie pull

**Uzavřené minimum:** pull je explicitní O1 akce s čerstvým preflightem, ochranou lokálních změn a resource-key serializací.  
...
**Termín:** před implementací stavové operace pull ve F3.
