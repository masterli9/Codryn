...
import type { FakeScenario } from './scripted-model-adapter.js';
...
export function changeVerifyReturnScenario(input: { readonly expectedHash: string; readonly projectRoot: string; readonly callIds?: Readonly<typeof callIds> }): FakeScenario {
  ...
  return {
    ...
    steps: [
      ...
      { assertRequest(request) { assertHistory(request, 6, 3); const patch = result(request, 2); if (patch.status !== 'applied' || patch.path !== 'sum.mjs') throw new Error('scenario'); }, events: [{ type: 'tool_call', call: { callId: ids.command, toolId: 'command.run', toolVersion: 1, arguments: { command: { executable: process.execPath, args: ['--test', 'sum.test.mjs'], cwd: input.projectRoot, timeoutMs: 30_000, maxOutputBytes: 256 * 1024 }, reason: 'Verify the repaired fixture.', impact: 'Runs the targeted project test once.' } } }, { type: 'completed' }] },
