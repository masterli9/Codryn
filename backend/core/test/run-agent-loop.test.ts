...
import { RunAgentLoop, ToolExecutionHarness, ToolRegistry, ControlledPermissionPolicy, fileReadTool } from '../src/index.js';
...
describe('RunAgentLoop', () => {
  it('orchestrates one completed adapter response per sequential step and persists only safe event metadata', async () => {
    ...
    const model: ModelAdapter = {
      ...
      stream(request) { requests.push(request); return requests.length === 1 ? stream({ type: 'tool_call', call: { callId, toolId: 'file.read', toolVersion: 1, arguments: { path: 'README.md' } } }, { type: 'completed' }) : stream({ type: 'text_delta', text: 'Hotovo.' }, { type: 'completed' }); }
    };
